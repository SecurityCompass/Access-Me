...
/**
 * Report Generator
 * The report generator is responsible for generating a report based on data
 * from a results manager
 * @param resultsManager a resultsManager that is to be mixed in to this
 *                       generator
 */
function SecCompReportGenerator(resultsManager){
    for (var key in resultsManager) {
        if (this[key] == undefined) {
            this[key] = resultsManager[key];
        }
    }
}

SecCompReportGenerator.prototype = {
    /**
    ...
    generateTopOfReport: function(testManager, errorstr) {
        ...
        this.results+= "</a>";
        this.results += "</div><div class='title'>Test Results</div></div>";
...
function generateMoreOfReportBody(reportGenerator, sortedResults, errorstr,
        timeout, resultIndex)
{
    reportGenerator.generateBodyOfReport(sortedResults, errorstr, timeout,
            resultIndex);
}